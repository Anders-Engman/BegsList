$(function() {
  ...
  $(document).on("click", "#submit-button", function(event) {
    ...
    //reason text
    var showReasonText = $("#reason-textarea")
      .val()
      .trim();
  ...
  function showSearchResults(item) {
    $.post("/api/searchItems", item, function(data) {
      ...
      similarItemsDiv.addClass(
        "row d-flex align-self-center flex-wrap text-center"
      );
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        chooseButton.addClass(
          "btn btn-info choose-button slidingUp-SearchItems"
        );
